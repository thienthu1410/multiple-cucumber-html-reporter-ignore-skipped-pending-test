...
let FEATURES_OVERVIEW_TEMPLATE_IGNORE_SKIPPED_TEST = 'components/features-overview-ignore-skipped-test.tmpl';
...
const SCENARIOS_TEMPLATE_IGNORE_SKIPPED_TEST = 'components/scenarios-ignore-skipped-test.tmpl';
const RESULT_STATUS = {
  passed: 'passed',
  failed: 'failed',
  skipped: 'skipped',
...
function generateReport(options) {
  ...
  const ignoreSkipTest = options.ignoreSkipTest || null;
  ...
  if ((!isNull(ignoreSkipTest) && ignoreSkipTest === false || isNull(ignoreSkipTest))){
...
=====================================================================================\n`)
  ...
  function _parseFeatures(suite) {
    suite.features.forEach((feature) => {
      ...
      if (!feature.isSkipped && !feature.isPending){
        suite.featureCount.total++;
      }

      if ((feature.isSkipped || feature.isPending) && (!isNull(ignoreSkipTest) && ignoreSkipTest === false || isNull(ignoreSkipTest))){
        suite.featureCount.total++;
      }
      ...
      if (feature.isFailed) {
        ...
        feature.notDefined++;
        suite.featureCount.notDefined++;
      } else if (feature.isPending && (!isNull(ignoreSkipTest) && ignoreSkipTest === false || isNull(ignoreSkipTest))) {
        feature.pending++;
        suite.featureCount.pending++;
      } else if (feature.isSkipped && (!isNull(ignoreSkipTest) && ignoreSkipTest === false || isNull(ignoreSkipTest))) {
        feature.skipped++;
        suite.featureCount.skipped++;
      } else if (!feature.isSkipped && !feature.isPending){
        feature.passed++;
        suite.featureCount.passed++;
      }
  ...
  function _parseSteps(scenario) {
    scenario.steps.forEach((step) => {
      ...
      if (step.result.status.toLowerCase() === RESULT_STATUS.pending && (!isNull(ignoreSkipTest) && ignoreSkipTest === false || isNull(ignoreSkipTest))) {
  ...
  function _createFeaturesOverviewIndexPage(suite) {
    ...
    if (!isNull(ignoreSkipTest) && ignoreSkipTest === false || isNull(ignoreSkipTest)) {
      ...
      FEATURES_OVERVIEW_TEMPLATE = suite.customMetadata
        ? FEATURES_OVERVIEW_CUSTOM_METADATA_TEMPLATE
        : FEATURES_OVERVIEW_TEMPLATE_IGNORE_SKIPPED_TEST;
    }
       ...
       fs.writeFileSync(
        ...
        fs.writeFileSync(
         ...
         _.template(_readTemplateFile(FEATURE_OVERVIEW_INDEX_TEMPLATE))({
           ...
           scenarioTemplate: _.template(_readTemplateFile(SCENARIOS_TEMPLATE_IGNORE_SKIPPED_TEST))({
             suite: suite,
             scenarios: feature.elements,
             _: _,
           }),
